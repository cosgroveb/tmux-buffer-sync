...
func (m *TmuxBufferSync) TestPlugin(ctx context.Context, source *dagger.Directory) error {
	_, err := dag.Container().
		...
		WithExec([]string{"tmux", "new-session", "-d", "-s", "test-session"}).
		WithEnvVariable("TMUX", "/tmp/tmux-1000/default,1234,0").
		WithExec([]string{"bash", "tests/test_end_to_end.sh"}).
		Stdout(ctx)
...
func (m *TmuxBufferSync) TestMultiServerSync(ctx context.Context, source *dagger.Directory) error {
	...
	server1Output, err := dag.Container().
		...
		WithExec([]string{"tmux", "new-session", "-d", "-s", "server1"}).
		WithEnvVariable("TMUX", "/tmp/tmux-1000/default,1234,0").
		WithExec([]string{"sh", "-c", "echo 'multi-server-test-content' | tmux load-buffer -"}).
		WithExec([]string{"bash", "-c", "source scripts/helpers.sh && source scripts/atuin_adapter.sh && push_buffers_to_atuin 'tmux-buffers-test' 1"}).
		Stdout(ctx)
	...
	server2Output, err := dag.Container().
		...
		WithExec([]string{"sh", "-c", "mkdir -p ~/.local/share/atuin && atuin init bash --disable-up-arrow --disable-ctrl-r"}).
		WithServiceBinding("shared-storage", sharedStorage).
		WithExec([]string{"tmux", "new-session", "-d", "-s", "server2"}).
		WithEnvVariable("TMUX", "/tmp/tmux-1000/default,1234,0").
		WithExec([]string{"bash", "-c", "source scripts/helpers.sh && source scripts/atuin_adapter.sh && pull_buffers_from_atuin 'tmux-buffers-test' 1"}).
		WithExec([]string{"sh", "-c", "tmux show-buffer | grep -q 'multi-server-test-content' && echo 'SUCCESS: Buffer synced between servers' || echo 'FAILED: Buffer not found'"}).
		Stdout(ctx)
